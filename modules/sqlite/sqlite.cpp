...
bool SQLite::open(String path) {
	...
	if (!Engine::get_singleton()->is_editor_hint() && path.begins_with("res://")) {
		...
		PoolVector<uint8_t> buffer = dbfile->get_buffer(size);
...
/*
  ...
  @return status
*/
bool SQLite::open_buffered(String name, PoolVector<uint8_t> buffers, int64_t size) {
	...
	memcpy(p_mem->data, buffers.write().ptr(), size);
	p_mem->data[size] = '\0';
	...
	for (int i = 0; i < param_count; i++) {
		...
		switch (args[i].get_type()) {
			...
			case Variant::Type::POOL_BYTE_ARRAY:
				retcode = sqlite3_bind_blob(stmt, i + 1, (args[i].operator PoolVector<uint8_t>()).write().ptr(), (args[i].operator PoolVector<uint8_t>()).size(), SQLITE_TRANSIENT);
				break;
			default:
...
void SQLite::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("get_error"), &SQLite::get_error);
}
